...
require_once __DIR__ . '/prompt_library.php';
...
function elevaro_teacher_ai_load_draft(int $draftId, int $teacherId): array
{
    ...
    if (!$draft) {
        throw new RuntimeException('Entwurf wurde nicht gefunden.');
    }

    $draftTeacherId = (int)($draft['teacher_id'] ?? 0);

    if ($teacherId > 0 && $draftTeacherId === $teacherId) {
        return $draft;
    }
    ...
    function elevaro_teacher_ai_confirm_analysis(int $draftId, int $teacherId, array $analysisUpdates): array
{
    ...
    $_SESSION['elevaro_ai_wizard_active_draft_id'] = $draftId;
...
if (!function_exists('elevaro_teacher_ai_split_build_questions_prompt')) {
    function elevaro_teacher_ai_split_build_questions_prompt(array $class, array $analysis, int $blockIndex, int $blockSize, string $mode, string $sourceText, string $extraPrompt, array $existingQuestions): string
    {
        ...
        if ($mode === 'listening') {
            ...
            $contextRules .= "- Die Übungen dürfen übernommen oder leicht variiert werden, ABER nur wenn die Quizfrage selbst vollständig lösbar ist.\n";
...
if (!function_exists('elevaro_teacher_ai_build_plausibility_prompt')) {
    function elevaro_teacher_ai_build_plausibility_prompt(array $analysis, array $questions, string $mode): string
    {
        ...
        $promptLibrary = function_exists('elevaro_prompt_library_build') ? elevaro_prompt_library_build([
            'stage' => 'plausibility',
            'mode' => $mode,
            'subject_label' => (string)($analysis['subject'] ?? $analysis['subject_label'] ?? ''),
