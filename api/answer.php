...
try {
    ...
    if (!is_array($payload)) throw new RuntimeException('Invalid JSON payload.');
    ...
    if (!$quizId || !$questionId) throw new RuntimeException('Missing quiz_id or question_id.');

    // Anonymous/global statistics must never block user progress.
    try {
    ...
    if ($userId) {
        $progress = elevaro_record_user_answer($userId, $quizId, $questionId, $selectedAnswer, $correctAnswer, $isCorrect, $sessionId, $points, $responseTimeMs);
    }
    echo json_encode(['success'=>true,'logged_in'=>(bool)$userId,'user_id'=>$userId,'progress'=>$progress], JSON_UNESCAPED_UNICODE);
} catch (Throwable $e) {
