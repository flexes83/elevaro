...
<div class="card card-soft admin-table-card"><div class="card-body p-4">
  ...
  <table class="table"><thead><tr><th>Name</th><th>PIN</th><th>Typ</th><th>Status</th><th>Zuletzt gesehen</th></tr></thead><tbody>
    <?php foreach ($participants as $student): $online = !empty($student['last_seen_at']) && strtotime((string)$student['last_seen_at']) >= time() - 180; ?>
      <tr><td><strong><?= teacher_h(($student['avatar_emoji'] ?? '🙂') . ' ' . $student['display_name']) ?></strong></td><td><?php if (empty($student['user_id']) && !empty($student['guest_pin'])): ?><span class="badge text-bg-dark font-monospace"><?= teacher_h($student['guest_pin']) ?></span><?php else: ?><span class="text-muted">–</span><?php endif; ?></td><td><?= !empty($student['user_id']) ? 'Schülerkonto' : 'Gast' ?></td><td><?= $online ? '<span class="badge text-bg-success">online</span>' : '<span class="badge text-bg-light">offline</span>' ?></td><td><?= teacher_h($student['last_seen_at'] ?: $student['created_at']) ?></td></tr>
    <?php endforeach; ?>
    <?php if (!$participants): ?><tr><td colspan="5" class="text-muted">Noch keine Schüler im Klassenraum. Teile den QR-Code oder Link.</td></tr><?php endif; ?>
  </tbody></table>
</div></div>
