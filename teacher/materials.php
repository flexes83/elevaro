...
$units = teacher_library_build_units($items, $classes);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'share_unit_class') {
            ...
            $stmt = $pdo->prepare('INSERT INTO teacher_unit_colleague_shares (unit_id, teacher_id, recipient_email, token, item_refs_json) VALUES (:unit_id, :teacher_id, :recipient_email, :token, :item_refs_json)');
            $stmt->execute([
                'unit_id' => $unitId,
                'teacher_id' => $teacherId,
            ...
            $url = teacher_library_absolute_url('/teacher/shared_unit.php?token=' . urlencode($token));
...
<?php if (!$units): ?>
  <div class="library-empty"><strong>Noch keine eigenen Materialien gespeichert.</strong>Starte mit „Neues Material erstellen“. Danach erscheinen deine Inhalte hier automatisch als Units.</div>
<?php else: ?>
    ...
    <?php foreach ($units as $unit): ?>
      ...
      <article class="unit-card" data-library-unit data-search="<?= teacher_h($searchText) ?>" data-format="<?= teacher_h($formatFilter) ?>" data-subject="<?= teacher_h(teacher_library_norm((string)$unit['subject_label'])) ?>" data-grade="<?= teacher_h(teacher_library_norm((string)$unit['grade'])) ?>" data-topic="<?= teacher_h(teacher_library_norm($topicText)) ?>">
        ...
        <div class="unit-body">
          ...
          <?php $hasManyItems = count($unit['items']) > 3; ?>
          <div class="unit-content-list<?= $hasManyItems ? ' is-collapsed' : '' ?>" data-unit-content-list>
            <?php foreach ($unit['items'] as $item): ?>
              <?php $itemUrl = trim((string)($item['edit_url'] ?: $item['url'] ?: $item['pdf_url'] ?: '#')); ?>
              <a class="unit-content-row" href="<?= teacher_h($itemUrl) ?>">
                <span class="unit-content-icon"><?= teacher_library_type_icon((string)$item['type']) ?></span>
                <span>
                  <strong><?= teacher_h((string)$item['title']) ?></strong>
                  <small><?= teacher_h(teacher_library_type_label((string)$item['type'])) ?><?= !empty($item['question_count']) ? ' · ' . (int)$item['question_count'] . ' Fragen' : '' ?></small>
                </span>
                <span class="unit-content-open">Öffnen</span>
              </a>
            <?php endforeach; ?>
          </div>
          <?php if ($hasManyItems): ?>
            <button class="unit-content-more" type="button" data-unit-content-more data-more-label="Alle Inhalte anzeigen" data-less-label="Weniger anzeigen">Alle Inhalte anzeigen</button>
          <?php endif; ?>
          <div class="unit-cta">
...
(() => {
  ...
  document.querySelectorAll('[data-unit-content-more]').forEach((button) => {
    button.addEventListener('click', () => {
      const list = button.previousElementSibling;
      if (!list) return;
      const expanded = !list.classList.toggle('is-collapsed');
      button.textContent = expanded ? (button.dataset.lessLabel || 'Weniger anzeigen') : (button.dataset.moreLabel || 'Alle Inhalte anzeigen');
    });
  });
